...
func _physics_process(_delta):
	if Input.is_action_pressed("ui_accept") and interact and !get_parent().get_parent().dialogue:
		...
		if get_parent().get_parent().check_flower and get_parent().get_parent().read_potion:
			var add = get_parent().get_parent().get_node("Cat").get_item(ash)
			if add:
				var dialog = Dialogic.start('GetAsh')
			...
			var dialog = Dialogic.start('CheckAsh')
			dialog.connect("dialogic_signal", self, "dialog_listener")
			add_child(dialog)
...
func _on_Ash_body_entered(_body):
	$Label.visible = true	
	interact = 1


func _on_Ash_body_exited(_body):
	$Label.visible = false
	interact = 0
